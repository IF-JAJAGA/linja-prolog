...
%Règle qui réalise l'affichage des jetons
print_GUI_Jetons([],_).
...
%Règle qui réalise l'affichage des jetons d'une zone du plateau
print_GUI_line([],_).
print_GUI_line([T1,T2|_],NumLine) :-
	print_GUI_pion(T1,0,NumLine,colour(red)),
	print_GUI_pion(T2,T1,NumLine,colour(black)).
	
%Règle qui réalise l'affichage des jetons d'une couleur d'une zone du plateau
...
%Désallocation des composants visuels
free_GUI_components :-
	free(@b),
	free(@r),
	free(@l),
